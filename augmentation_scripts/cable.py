...
def create_augmentation_pipeline(aug_type, severity=None):
    ...
    if aug_type == "rotate":
        ...
        if flip_type == "h":
            ...
            ], additional_targets={'mask': 'mask'}), "fh"
        else:
            return A.Compose([
                A.VerticalFlip(p=1.0)
            ], additional_targets={'mask': 'mask'}), "fv"
            
    elif aug_type == "scale":
        scale = random.uniform(*scale_range)
        return A.Compose([
            A.Affine(scale=scale, p=1.0)
        ], additional_targets={'mask': 'mask'}), f"{scale:.2f}"
        ...
        ], additional_targets={'mask': 'mask'}), f"{contrast:.1f}_{int(brightness*100)}"
        
    elif aug_type == "perspective":
        scale = random.uniform(*perspective_range)
        return A.Compose([
            A.Perspective(scale=scale, p=1.0, keep_size=True, pad_mode=cv2.BORDER_CONSTANT)
        ], additional_targets={'mask': 'mask'}), f"{int(scale*100)}"
        ...
        ], additional_targets={'mask': 'mask'}), f"h{int(hue)}_s{int(saturation*100)}_v{int(value*100)}"
        ...
        return A.Compose([
            A.Rotate(limit=(angle, angle), p=1.0, border_mode=cv2.BORDER_CONSTANT),
            A.Perspective(scale=persp_scale, p=1.0, keep_size=True, pad_mode=cv2.BORDER_CONSTANT),
            A.RandomBrightnessContrast(
                brightness_limit=(brightness, brightness),
                contrast_limit=(contrast, contrast),
                p=1.0
            )
        ], additional_targets={'mask': 'mask'}), f"mix_r{int(angle)}_p{int(persp_scale*100)}_b{contrast:.1f}_{int(brightness*100)}"
        ...
        return A.Compose([
            ...
            A.Affine(scale=scale, p=1.0)
        ], additional_targets={'mask': 'mask'}), f"cable_r{int(angle)}_h{int(hue)}_s{scale:.2f}"
        
    else:
        # Default to a slight rotation as fallback
        return A.Compose([
            A.Rotate(limit=10, p=1.0, border_mode=cv2.BORDER_CONSTANT)
        ], additional_targets={'mask': 'mask'}), "default"
...
def apply_augmentation(image_data, target_count):
    ...
    # Create debug directory for saving augmentation validation 
    debug_dir = "debug_augmentation"
    ...
    for base_name, img_path, mask_path in zip(base_names, img_paths, mask_paths):
        ...
        # Debug visualization for original images
        orig_image = cv2.imread(img_path)
        orig_mask = cv2.imread(mask_path, cv2.IMREAD_GRAYSCALE)
        
        # Create debug visualization
        debug_image = cv2.cvtColor(orig_image.copy(), cv2.COLOR_BGR2RGB)
        debug_mask = np.zeros_like(debug_image)
        debug_mask[orig_mask == CABLE_CLASS] = [0, 0, 255]  # Blue for cable
        debug_overlay = cv2.addWeighted(debug_image, 0.7, debug_mask, 0.3, 0)
        debug_file = os.path.join(debug_dir, f"debug_original_{base_name}.jpg")
...
def visualize_samples(num_samples=5):
    ...
    if num_samples == 1:
        axes = np.array([axes])  # Ensure axes is always 2D for indexing
    
    for i, img_file in enumerate(samples):
        # Get corresponding mask file - need to handle naming correctly
        base_name = os.path.splitext(img_file)[0]  # Remove extension
        mask_file = f"{base_name}_semantic_mask.png"
        ...
        if not os.path.exists(mask_path):
            print(f"Mask not found for {img_file}, checking alternative path format...")
            # Try alternative mask path format
            mask_file = f"{base_name.split('_flip')[0]}_flip_{base_name.split('_flip_')[1]}_semantic_mask.png" if "_flip_" in base_name else f"{base_name}_semantic_mask.png"
            mask_path = os.path.join(OUTPUT_MASK_DIR, mask_file)
            if not os.path.exists(mask_path):
                print(f"Still no mask found for {img_file}")
                continue
        ...
        if image.shape[:2] != mask.shape[:2]:
            print(f"Warning: Image and mask dimensions don't match for {img_file}")
        ...
        axes[i, 0].set_title(f"Augmented Image: {os.path.basename(img_file)}")
        axes[i, 0].axis('off')
        
        axes[i, 1].imshow(overlay)
        axes[i, 1].set_title(f"Cable Overlay (Mask: {os.path.basename(mask_file)})")
        axes[i, 1].axis('off')
