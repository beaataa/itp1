...
def create_augmentation_pipeline(aug_type, severity=None):
    ...
    if aug_type == "rotate":
        ...
        # Use RandomBrightnessContrast which properly handles both brightening and darkening
        return A.Compose([
            A.RandomBrightnessContrast(
                brightness_limit=(brightness, brightness),  # Fixed value, not a range
                contrast_limit=(contrast, contrast),  # Fixed value, not a range
                p=1.0
            )
